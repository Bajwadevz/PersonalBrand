    ...
    <>
        ...
        >
              ...
              return (
                <Link
                  ...
                  className={`group relative whitespace-nowrap shrink-0 text-sm font-medium transition-opacity duration-300 ${
                    isActive
                      ? "text-[var(--color-site-text)]"
                      : "text-[var(--color-site-text-secondary)] hover:text-[var(--color-site-text)]"
                  }`}
                >
                  {label}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="hidden lg:inline-flex min-h-[40px] items-center rounded-full bg-[#2D6A8F] px-4 text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#245A7A] whitespace-nowrap"
            >
              Book an Audit
            </Link>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <nav className="mx-auto flex min-h-[60vh] max-w-sm flex-col items-center justify-center gap-6 text-center">
              {navItems.map(({ href, label }) => {
                const isActive = pathname === href || (href !== "/" && pathname.startsWith(href));
                return (
                  <Link
              ...
              <Link
                href="/contact?offer=lead-flow-audit"
                onClick={() => setMobileOpen(false)}
                className="inline-flex min-h-[48px] w-full items-center justify-center rounded-full bg-[#2D6A8F] px-6 text-base font-semibold text-white"
              >
                Book an Audit
              </Link>
            </nav>
